...
{
    friend std::ostream& operator<<(std::ostream&, const LGInterpolator&);
    public:
...
{
    friend std::ostream& operator<<(std::ostream&, const LGECInterpolator&);
    public:
        LGECInterpolator (const ECPointsList&, const BigNumber& ) ; 
        ~LGECInterpolator () = default;

        ECPoint operator()( const BigNumber&, const int&);
        BigNumber operator()(const int&, const BigNumber&);
