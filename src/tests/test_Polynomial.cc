 ...
 */
TEST(POLYNOMIALTest, test_polynomial_degree1){
    // 3 + 2x 
    ...
    EXPECT_EQ ( degree, strCoefficients.size() - 1 ) ;
    ...
    EXPECT_EQ  ( eval_at_0.ToDec(), "3"  ) ;
    EXPECT_EQ  ( eval_at_1.ToDec(),  "5"  ) ;

    std::ostringstream ss ;
    ss << poly ;
    EXPECT_EQ ( ss.str(), "modulo = 0;    polynomial = 3 + 2x" ) ;
}
 ...
 *   Check the coefficients
 */
TEST(POLYNOMIALTest, test_polynomial_degree2 ){
     // 1 + 2x + 3x^2 
    ...
    EXPECT_EQ  ( eval_at_0.ToDec( ), "1" ) ;
    EXPECT_EQ  ( eval_at_1.ToDec( ), "6" ) ;
    EXPECT_EQ  ( eval_at_2.ToDec( ),  "17" ) ;

    std::ostringstream ss ;
    ss << poly ;
    EXPECT_EQ ( ss.str( ),  "modulo = 0;    polynomial = 1 + 2x + 3x^2" ) ;
}
 ...
 */
TEST(POLYNOMIALTest, test_polynomial_degree2_mod){
    // 6 + 7x + 8x^2  [mod 5]
    ...
    EXPECT_EQ  ((poly[0] ),  array_0) ;
    EXPECT_EQ  ((poly[1]), array_1) ;
    EXPECT_EQ  ((poly[2]), array_2);
    ...
    EXPECT_EQ  ( eval_at_0.ToDec( ), "1" ) ;
    EXPECT_EQ  ( eval_at_1.ToDec( ), "1" ) ;
    EXPECT_EQ  ( eval_at_2.ToDec( ), "2" ) ;

   
    std::ostringstream ss ;
    ss << poly ;
    EXPECT_EQ ( ss.str( ),  "modulo = 5;    polynomial = 1 + 2x + 3x^2" ) ;
}
 ...
 */
TEST(POLYNOMIALTest,test_polynomial_degree2_mod2 ){
    // 16 + 9x^2  [mod 5]
    ...
    EXPECT_EQ  ( eval_at_0.ToDec( ), "1" ) ;
    EXPECT_EQ  ( eval_at_1.ToDec( ), "0" ) ;
    EXPECT_EQ  ( eval_at_2.ToDec( ),  "2" ) ;

    std::ostringstream ss ;
    ss << poly ;
    EXPECT_EQ ( ss.str( ),  "modulo = 5;    polynomial = 1 + 0x + 4x^2" ) ;    
}
 ...
 */
TEST(POLYNOMIALTest,test_polynomial_random1 ){
    ...
    EXPECT_EQ( Polynomials.back( ).getDegree( ),5) ;
    ...
    EXPECT_EQ( Polynomials.back( ).getDegree( ), 2 ) ;
    
    // test the a_0 is as expected
    EXPECT_EQ( a_0,  Polynomials.back( )[ 0 ] ) ;
    ...
    for( 
        ...
        EXPECT_FALSE((*poly)[0] == GenerateZero());
        // test last term is not zero
        EXPECT_FALSE(( (*poly)[ poly->getDegree( ) ] == GenerateZero( ) ) ) ;
...
TEST(POLYNOMIALTest, test_polynomial_hide ){
    ...
    EXPECT_EQ( hiddenPoly[2], hiddenPoly[3]) ;
    EXPECT_EQ( hiddenPoly[0].ToDec(), "344295838914342996903890629191305488808654946672820536623545781571036035167993" ) ;
}
...
*/
TEST(POLYNOMIALTest, test_polynomial_stress_1 ){
    ...
    for ( auto i = 0 ; i < 200 ; ++i ){
        ...
        EXPECT_TRUE ( eval_randomX1 > minusOne ) ;
        EXPECT_TRUE ( eval_randomX1 <  modulo ) ;
        EXPECT_TRUE ( eval_randomX2 > minusOne ) ;
        EXPECT_TRUE ( eval_randomX2 < modulo ) ;        
    }
}
 ...
 */
TEST(POLYNOMIALTest, test_polynomial_stress_2 ){
    ...
    for ( auto counter = 0; counter < 200; ++counter ){
        ...
        EXPECT_EQ(result_manual,result_polynomial) ;
    }
}
 ...
 */
TEST(POLYNOMIALTest, test_polynomial_stress_3){
    BigNumber modulo, min, max ;
...
TEST(POLYNOMIALTest, test_polynomial_min_max )
{
    // Invalid range, min == max == 0
    EXPECT_THROW( Polynomial(3, GenerateZero()), std::range_error);
...
// Check a_0 is zero
TEST(POLYNOMIALTest, test_polynomial_a_0_zero ){
    // a_0 coefficient should be strictly positive, a_0 = 0
    EXPECT_THROW(Polynomial( 3, GenerateZero( ), GenerateZero( ) ), std::range_error);
    ...
    EXPECT_THROW(Polynomial(bnCoefficients, GenerateZero()),std::range_error);        
}

// Check a_0 > modulo
TEST(POLYNOMIALTest, test_polynomial_a_0_modulo){
    BigNumber modulo, a_0 ;
    modulo.FromDec( "5" ) ;
    a_0.FromDec( "6" ) ;

    // a_0 coefficient should be lower than modulo, but a_0=6, modulo=5
    EXPECT_THROW( Polynomial( 3, modulo, a_0  ) ,std::range_error);
} 

// Check min >= 0 
TEST(POLYNOMIALTest, test_polynomial_min_zero){
    BigNumber min, max ;
    min.FromDec( "-1" ) ;
    max.FromDec( "1" ) ;

    // min should be zero or greater, min = -1
    EXPECT_THROW( Polynomial( 3, GenerateZero( ), min, max ) ,std::range_error );
}

// Check min >= 0 
TEST(POLYNOMIALTest,test_polynomial_max_modulo ){
    BigNumber modulo, min, max ;
    modulo.FromDec( "5" ) ;
    min.FromDec( "0" ) ;
    max.FromDec( "4" ) ;
    ...
    EXPECT_THROW(Polynomial( 3, modulo, min, max ) ,std::range_error);
}

// Check empty coefficients
TEST(POLYNOMIALTest,test_polynomial_empty_coeff ){
   std::vector< BigNumber >    bnCoefficients ; ;     

    // Polynomial is empty, returning
    EXPECT_THROW (Polynomial poly ( bnCoefficients, GenerateZero( ) ) ,std::runtime_error);
}


// Check zero coefficients at highest degree
TEST(POLYNOMIALTest,test_polynomial_zero_high ){
    ...
    EXPECT_THROW( Polynomial poly ( bnCoefficients, GenerateZero( ) ) ,std::range_error);          
}
...
TEST(POLYNOMIALTest, test_Interpolation_degree_3_mod_17 ){
    int degree = 3;
    BigNumber mod; 
    mod.FromDec("17"); 
    ...
    for(auto testIter = xfx.begin(); testIter != xfx.end(); ++ testIter){
        BigNumber TestVal = interpFunc(testIter->first);
        EXPECT_EQ (TestVal.ToDec(),testIter->second.ToDec());
    }
}

// Test creation of LGInterpolator object of degree 100, modulo 104729
TEST(POLYNOMIALTest, test_Interpolation_degree_100_mod_104729 ){
    ...
    LGInterpolator interpFunc(xfx, mod);
     ...
     for(std::vector<std::pair<BigNumber, BigNumber> >::const_iterator testIter = xfx.begin(); testIter != xfx.end(); ++ testIter){
        BigNumber TestVal = interpFunc(testIter->first);
        EXPECT_TRUE (TestVal == testIter->second);
     }

}

// Test creation of LGInterpolator object of degree 200, modulo SECP256K1MOD
TEST(POLYNOMIALTest,test_Interpolation_degree_8_mod_SECP256K1MOD ){
    int degree = 7;
    BigNumber mod; 
...
TEST(POLYNOMIALTest,test_Interpolation_empty_coeff ){
    std::vector< std::pair<BigNumber,BigNumber > >   bnCoefficients ;      
    BigNumber Mod; 
    Mod.Zero(); 

    // Polynomial is empty, returning
    EXPECT_THROW(LGInterpolator interp ( bnCoefficients, Mod ), std::runtime_error);
}

TEST(POLYNOMIALTest, test_Interpolation_repeated_coeff ){
...
TEST(POLYNOMIALTest, test_Interpolation_eval_at_invalid_basis){
    int degree = 7;
    BigNumber mod; 
    mod.FromHex("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141"); 
    ...
    EXPECT_THROW( BigNumber valBasis = interpFunc(55, xValue), std::runtime_error);

    EXPECT_THROW(BigNumber valBasis = interpFunc(-1, xValue),std::runtime_error);
}
////
TEST(POLYNOMIALTest, test_Interpolation_eval_at_basis){
    int degree = 3;
    BigNumber mod; 
    mod.FromHex("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141"); 
    ...
    for (std::vector<std::pair<BigNumber, BigNumber> >::const_iterator iter = xfx.begin (); iter != xfx.end(); ++ iter){
        BigNumber ithVal = interpFunc(index--, iter->first);
        EXPECT_TRUE(ithVal == zero);
    }

}

TEST(POLYNOMIALTest, test_ECPoint_Interpolation_degree_7_mod_SECP256K1CURVE){
    ...
    for(std::vector<std::pair<BigNumber, ECPoint> >::iterator testIter = curve.begin(); testIter != curve.end(); ++ testIter){
        ECPoint TestVal = interpolator(testIter->first, 714);
        EXPECT_TRUE (TestVal == testIter->second);
     }
}
...
TEST(POLYNOMIALTest,  test_ECPoint_Interpolation_BNInterpolattionMuLG_EQUAL ){
    int degree = 10;
    BigNumber mod; 
    mod.FromHex("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141"); 
    ...
    ECPoint ecPointAtZero = interpolator(xValueAtZero, 714);

    
    // multiply bn*G
    
    ECPoint bnAtZero = G * InterpolatedAtZero;
    
    
    EXPECT_TRUE (bnAtZero == ecPointAtZero);
}
