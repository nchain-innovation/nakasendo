...
class ECPointImpl
{
    ...
    public:
        ...
        // Multiply ECPoint with BigNumber type
        std::unique<ECPointImpl> Multiply(const ECPointImpl& ec_pt, const BigNumber& bn) const; 
        ...
        void SetRandom ();
        std::pair<std::string, std::string> GetAffineCoords () const ;
        std::pair<std::string, std::string> GetAffineCoords_Dec () const ;
...
std::string GroupOrderImpl(const int&);
